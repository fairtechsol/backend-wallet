...
class GrpcClient {
  ...
  loadProtoServices(protoOptionsArray) {
    ...
    protoOptionsArray.forEach((protoOptions) => {
      ...
      ](this.serverAddress, grpc.credentials.createSsl(fs.readFileSync(`/etc/letsencrypt/live/betfair.fairgame.club/fullchain.pem`), fs.readFileSync("/etc/letsencrypt/live/betfair.fairgame.club/privkey.pem"),fs.readFileSync(`/etc/letsencrypt/live/betfair.fairgame.club/chain.pem`)))
    });
    return clients;
  }
